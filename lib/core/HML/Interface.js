 ...
 HML.Interface = function() {
  ...
  Interface.IdListInput.addEventListener('click', function() {
   ...
   Interface.IdListInput.FileSelect.addEventListener('change', function(e) {
    if (Interface.IdListInput.FileSelect.files.length) {
     ...
     Interface.FeaturesObject = {};
     Interface.FeaturesArray = [];
     Interface.IdFile = new MSLIB.Format.TextTableFile(e.target.files[0]);
    ...
    if (peptide) {
     ...
     if (fname) {
      if (fname && (Interface.AvailableFileNames.indexOf(fname) < 0)) {
       ...
       MSLIB.Common.waitUntil(() => Interface.Files[fname].Worker.Ready,() => {
        var ms1rt = Interface.Files[fname].Response;
        ...
        MSLIB.Common.waitUntil(() => Interface.Files[fname].Worker.Ready,() => {
         var idrt = Interface.Files[fname].Response;
  ...
  var handleProcessClick = function(e) {
   ...
   if (
    ...
    window.requestAnimationFrame(() => {MSLIB.Common.waitUntil(() => true, startProcessing.bind(null,e.target.SelectedOnly))}); 
   }
  }
  ...
  var TaskQueue = function() {
   var TQ = [];
   TQ.add = function(QuantTask) {
    if (this.length < (Interface.QuantWorkers.length*10)) {
     this.push(QuantTask);
     this.update();
     return true;
    }
   ...
   TQ.update = function() {
    var nextAvailableWorker = Interface.QuantWorkers.find(QW => QW.Worker.Ready);
    if (nextAvailableWorker && this.length) nextAvailableWorker.process(this.shift()); 
   }
   return TQ;
  }
  ...
  var startProcessing = function(SelectedOnly) {
   ...
   Interface.AvailableFileNames.forEach(fn => {
    P.Completed[fn] = 0;
    Interface.Files[fn].quantify(fileQRs[fn]);
   });
  ...
  var monitorProcessing = function() {
   ...
   if (Done < P.ToDo) {
    if (!P.SelectedOnly && Done) setProgressBar((Done/P.ToDo)*100);
    window.setTimeout(monitorProcessing, UI_UPDATE_INTERVAL);
   }
   else {
    hide(Interface.ProgressOverlay);
...
//    enable(Interface.DownloadProteins);
    ...
    console.log("Processing completed in "+(window.performance.now()-P.StartTime)+" ms");
  ...
  Interface.addQuantResult = function(QuantRequest,Quant,QuantExtended) {
   ...
   if (QuantExtended) HML.Interface.FeaturesObject[Protein][Peptide][Charge][Modstring][12][FileName].push(QuantExtended);
  }
  ...
  var displayCanvas = function() {
   ...
   ["unlabelled","heavy"].forEach(function(label,label_i) {
    if (MS1Feature[0][label_i].length) {
...
//   var stats = {};
//   var rows = [["PROTEIN"].concat(Array.concat.apply([],Interface.AvailableFileNames.map((fname) => 
//               (labels.map((label) => 
...
//    var quantbyfile = Interface.AvailableFileNames.map(function(fname) {
//     stats[fname] = {};
...
//       return Interface.FeaturesObject[protein][peptide].filter((F) => ((F.fn == fname) && F.Unique)).map(function(F) {
...
//        if (F.Origfn == fname) stats[fname][label].Distance.push((F.Results && !F.Results.Quant[label].Failed) ? F.IDRT - F.Results.Quant[label].RT : null);
...
//  Interface.DownloadProteins.addEventListener('click', downloadProteins);
  ...
  var downloadPeptides = function(e) {
   ...
   setProgressBar(0);
   var labels = ["unlabelled","heavy"];
   var rows = [["PROTEIN","PEPTIDE","IS_UNIQUE","CHARGE","MODIFICATIONS"].concat(Array.prototype.concat.apply([],Interface.AvailableFileNames.map(fname => Array.prototype.concat.apply([],labels.map(label => ["_INTENSITY_","_INCORP_","_SCORE_"].map(field => fname+field+label)))))).join("\t")]
     ...
     Array.prototype.concat.apply([],Object.keys(Interface.FeaturesObject[protein][peptide]).sort().map(charge =>
      Array.prototype.concat.apply([],Object.keys(Interface.FeaturesObject[protein][peptide][charge]).sort().map(modstring =>
       [protein,
        ...
        if (Interface.FeaturesObject[protein][peptide][charge][modstring][12] && Interface.FeaturesObject[protein][peptide][charge][modstring][12][fname]) {
         return Array.prototype.concat.apply([],[0,1].map(Li => Interface.FeaturesObject[protein][peptide][charge][modstring][12][fname][0][Li].map((n,i)=>n.toFixed(Precision[i]))))
        }
        else {
         return Array(6).fill("");
        }
