 ...
 HML.Interface = function() {
  ...
  var waitForIndexingComplete = function() {
   if ((Interface.FileTabs.childNodes.length ==  Interface.RawFiles.FileSelect.files.length) && (Object.keys(Interface.AllFiles).every(function(fname) { return Interface.AllFiles[fname].Ready }))) {
    Interface.RawFiles.Ready = 1;
    if (Interface.FileTabs.childNodes.length == 1) MSLIB.Common.WaitUntil(function() {return true},function() {Interface.FileTabs.childNodes[0].click();enable(Interface.IdList)});
  ...
  Interface.IdList.addEventListener('click', function() {
   ...
   Interface.IdList.FileSelect.addEventListener('change', function(e) {
    ...
    if (Interface.IdList.FileSelect.files.length) {
     Interface.IdList.idFile = new MSLIB.Format.TextTableFile(e.target.files[0]);
     Interface.IdList.idFile.Delimiter = "\t";
     Interface.IdList.idFile.UseFirstLineAsHeaders = 1;
     Interface.IdList.idFile.load();
     MSLIB.Common.WaitUntil(function(){return Interface.IdList.idFile.Ready},function(){
     ...
     MSLIB.Common.WaitUntil(function(){return Interface.IdList.Ready},function() {
  ...
  var processTicker = function() {
   ...
   if (p.FeatureIndex >= p.Features.length) {
    ...
    extractions.unlabelled = [new MSLIB.IsoCalc.Peptide({sequence:sequence,charge:charge}).get_centroided_distribution(10).as_spectrum()];
    ...
    for (var N_15 = 95; N_15 < 100; N_15++) {
     ...
     extractions.heavy.push(new MSLIB.IsoCalc.Peptide({sequence:sequence,charge:charge}).get_centroided_distribution(10,altEleConst).as_spectrum());
    }
    ...
    p.FeatureIndex++;
    MSLIB.Common.WaitUntil(function() {return true}, processTicker);  
   }
  };
