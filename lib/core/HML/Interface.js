 ...
 HML.Interface = function() {
     ...
     if (fname) {
      if (fname && (Interface.AvailableFileNames.indexOf(fname) < 0)) {
       ...
       if (!Interface.FeaturesObject[protein][peptide][charge][modstring]) {
        ...
        Interface.FeaturesObject[protein][peptide][charge][modstring][6].push(fname);
        Interface.FeaturesObject[protein][peptide][charge][modstring][7].push(ms1rt);
        Interface.FeaturesObject[protein][peptide][charge][modstring][8].push(idrt);
       }
      }
     }
    }
     ...
     Object.keys(Interface.FeaturesObject[protein][peptide]).sort().forEach(function(charge) {
      Object.keys(Interface.FeaturesObject[protein][peptide][charge]).sort().forEach(function(modstring) {
       ...
       tbodyrow.addEventListener('click', function(e) {
        ...
        Interface.SelectedFeature = tbodyrow.featureData;
  ...
  var handleProcessClick = function(e) {
   ...
   if (
    ...
    window.requestAnimationFrame(() => {window.requestAnimationFrame(startProcessing)});
   }
  }
  ...
  var processTicker = function() {
   ...
   if (!feature[13][filename]) {
    ...
    var MS1RT;
    if (MS1RTindex >= 0) {
     MS1RT = feature[7][MS1RTindex]
    }
    else {
     MS1RT = feature[7].reduce((a,b) => (a+b))/feature[7].length;
    }
  ...
  var displayCanvas = function() {
   ...
   var idrt = (Interface.SelectedFeature[6].indexOf(Interface.SelectedFile.fname) >= 0 
...
//    return labels.map((label) =>(fname+"_MEAN_INCORPORATION_"+label))
