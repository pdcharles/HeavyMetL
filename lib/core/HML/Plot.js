...
HML.Plot = function() {
 ...
 var MS1FeatureRelAbundance = function(feature,labels) {
  ...
  var quant = labels.map((lb,i) => (feature.Quant[i].length ? feature.Quant[i][1] : 0));
  var gwidth = 300;
  var gheight = 200;
  var xlmarg = 80;
  var xrmarg = 40;
 ...
 var MS1FeatureChromatogram = function(feature,labels,idrt) {
  ...
  var chromatograms = labels.map((lb,i) => (feature.Quant[i].length 
                                             ? new MSLIB.Data.Chromatogram(
                                                feature.QuantExtended[i].Chromatogram_rts,
                                                feature.QuantExtended[i].Chromatogram_ints
                                               )
                                             : null));
  chromatograms.forEach(function(chr,i) {
   if (chr) {
    chr.Label = labels[i];
    chr.QERT = feature.QuantExtended[i].RT;
    chr.QEIntensity = feature.QuantExtended[i].Intensity;
   }
  });
 ...
 var MS1FeatureSpectrum = function(feature,label,label_i) {
  ...
  var spectrum = new MSLIB.Data.Spectrum(
                      feature.QuantExtended[label_i].Spectrum_mzs,
                      feature.QuantExtended[label_i].Spectrum_ints
                     );
  var scaled_spectrum = new MSLIB.Data.Spectrum(
                             feature.QuantExtended[label_i].ScaledSpectrum_mzs,
                             feature.QuantExtended[label_i].ScaledSpectrum_ints
                            );
  var gwidth = 300;
  var gheight = 200;
  var xlmarg = 80;
  ...
  for (var i in scaled_spectrum.mzs) {
   var err = MSLIB.Math.ppmError(scaled_spectrum.mzs[i],feature.Params.PpmError);
  ...
  var titletxt = new fabric.Text("Label: "+label+" ("+feature.QuantExtended[label_i].Incorporation+"%)   -   idotp: "+feature.QuantExtended[label_i].Score.toFixed(2), {textAlign: "center", fontSize: tickfontsize, fontFamily:tickfontfamily, left: gleft+(xmax-xmin)/2*xscale, top: gtop+gheight+tickmarkerlength*3, originX: "left", originY: "top"});
