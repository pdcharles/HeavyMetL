...
HML.Plot = function() {
 ...
 var MS1FeatureRelAbundance = function(feature,labels) {
  ...
  var max_int = Math.max.apply(null, quant) || 0;
  ...
  for (var yt = 0; yt <= yticks; yt++) {
   ...
   var txt = new fabric.Text((yt*ytick).toExponential(HML.Config.DisplayYAxisDecimalPlaces), {textAlign: "left", fontSize: tickfontsize, fontFamily:tickfontfamily, left: gleft-20, top: gtop+gheight-y, originX: "left", originY: "top"});
 ...
 var MS1FeatureChromatogram = function(feature,labels,idrts) {
  ...
  var chromatograms = labels.map((lb,i) => (feature[i*4] 
                                             ? new MSLIB.Data.Chromatogram(
                                                feature[8][i].Chromatogram_rts,
                                                feature[8][i].Chromatogram_ints
                                               )
                                             : null));
  ...
  var xmin = min_rt - xtick/2;
  var xmax = max_rt + xtick/2;
  var xticks = Math.ceil((xmax-xmin)/xtick);
  xmax = xticks*xtick;
  var xscale = gwidth/xmax;
  var int_max = Math.max.apply(null, chromatograms.map((chr) => chr.getMaxIntensity()));
  var log_int_max = Math.log10(int_max || 1);
  var halfceil_log_int_max = Math.ceil(log_int_max)-0.5;
  var log_int_upper_bound = ( log_int_max < halfceil_log_int_max ? halfceil_log_int_max : Math.ceil(log_int_max) );
  var ymax,yticks;
  if (HML.Config.DisplayChromatogramIntensitiesLogged) {
   ymax = yticks = log_int_upper_bound;
  }
  else {
   ymax = Math.pow(10,log_int_upper_bound);
   yticks = 10;
  }
  ...
  if (idrts != null && idrts.length) {
   idrts.forEach(function(idrt) {
    ...
    g.add(idrtpath);
    var txt = new fabric.Text("ID ("+idrt.toFixed(2)+")", {fill: "rgb(255,0,0)", textAlign: "left", fontSize: labelfontsize, fontFamily:labelfontfamily, left: gleft+((idrt-xmin)*xscale), top: gtop+gheight-ymax*yscale-2*(labelheight+labeloffset), originX: "left", originY: "top"});
    txt.set({left: txt.left-(txt.height/2), top: txt.top-(txt.height/2), angle: -45});
    g.add(txt);
   });
  }
  ...
  for (var yt = 0; yt <= yticks; yt++) {
   ...
   var ytickval_label = (HML.Config.DisplayChromatogramIntensitiesLogged ? Math.pow(10,ytickval).toExponential(0) : ytickval.toExponential(HML.Config.DisplayYAxisDecimalPlaces));
   var txt = new fabric.Text(ytickval_label, {textAlign: "left", fontSize: tickfontsize, fontFamily:tickfontfamily, left: gleft-20, top: gtop+gheight-y, originX: "left", originY: "top"});
   txt.set({left: txt.left-txt.width-tickmarkerlength/5, top: txt.top-(txt.height/2)});
   g.add(txt);
   if (yt) {
    yaxisstr += " L 0 -"+y+" M 0 -"+y+" L -"+tickmarkerlength+" -"+y+" M 0 -"+y+"";
 ...
 var MS1FeatureSpectrum = function(feature,label,label_i) {
  ...
  var spectrum = new MSLIB.Data.Spectrum(
                      feature[8][label_i].Spectrum_mzs,
                      feature[8][label_i].Spectrum_ints
                     );
  var scaled_spectrum = new MSLIB.Data.Spectrum(
                             feature[8][label_i].ScaledSpectrum_mzs,
                             feature[8][label_i].ScaledSpectrum_ints
                            );
  var gwidth = 300;
  var gheight = 200;
  var xlmarg = 80;
  ...
  var titletxt = new fabric.Text("Label: "+label+" ("+feature[label_i*4+1]+"%)   -   Score: "+feature[label_i*4+3].toFixed(4), {textAlign: "center", fontSize: tickfontsize, fontFamily:tickfontfamily, left: gleft+(xmax-xmin)/2*xscale, top: gtop+gheight+tickmarkerlength*3, originX: "left", originY: "top"});
