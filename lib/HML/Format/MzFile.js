...
if (typeof zpipe != 'undefined') HML.Format.MzFile = function() {
 ...
 MzFile.prototype.fetchScanHeader = function(scan,prefetchSpectrumData) {
  ...
  if (this.Scans[scan].Scan) {
   ...
   this.CurrentScan.PrefetchSpectrum = prefetchSpectrumData;
 ...
 MzFile.prototype.fetchAllScanHeaders = function() {
  ...
  this.Progress = 0;
  this.CurrentScan = new HML.Data.Scan();
  this.Internal.FetchAll = 1;
  this.Reader.TextBuffer = "";
  this.Reader.readText(
   filetype[this.FileType].parseScanHeader,
   this.Scans[this.getFirstScanNumber()].Offset,
   HEADER_SLICE_SIZE
  );
 };
 ...
 var parseScanOffsetStart = function() {
  var regexmatch = regex[this.FileType].Index.exec(this.result);
  if (regexmatch) {
   this.MzF.Internal.Offsets.Index = +regexmatch[1];
   this.readText(
    filetype[this.FileType].parseScanOffsetList,
    this.MzF.Internal.Offsets.Index
   );
  }
  else {
   if (this.MzF.FileType == "mzXML") {
 ...
 var parseUnindexedScanOffsets = function() {
  ...
  while ((regexmatch = RE.exec(text)) !== null) {
   this.MzF.Scans[regexmatch[1]] = (text_offset + RE.lastIndex - regexmatch[0].length);
   end_scannum_index = RE.lastIndex;
  }
  text = text.substr(end_scannum_index);
  if (text.match(/<\/mzXML>/)) {
   this.MzF.Progress = 100;
   this.MzF.Ready = 1;
  }
  else {
 ...
 var parseScanOffsetList = function() {
  ...
  if (end_offset_index != 8) {
   ...
   for (var i = 0; i < offsets.length-1; i++) {
    var regexmatch = regex[this.FileType].ScanOffset.exec(offsets[i]);
    if (regexmatch) {
     this.MzF.Scans[+regexmatch[1]] = {};
     this.MzF.Scans[+regexmatch[1]].Offset = +regexmatch[2];
     if (previousScan) {
      this.MzF.Scans[previousScan].Length = this.MzF.Scans[+regexmatch[1]].Offset - this.MzF.Scans[previousScan].Offset;
      this.MzF.Scans[previousScan].Next = +regexmatch[1];
      this.MzF.Scans[+regexmatch[1]].Previous = previousScan;
     }
  ...
  parseScanHeader : function() {
   ...
   for (var i = 0; i < eles.length-1; i++) {
    ...
    if (/<spectrum /.exec(eles[i])) {
     ...
     if (regexmatch) {
      this.MzF.CurrentScan.BinaryDataLength.push(+regexmatch[1]);
     }
    }
     ...
     this.MzF.CurrentScan.BinaryDataOffset.push(this.Position - text.length + eles.slice(0,i+1).join("").length + i + 1)   
     if (!this.MzF.CurrentScan.BinaryDataOffset[1]) {
      if (this.MzF.CurrentScan.BinaryDataLength[0]) {
       this.Position = +this.MzF.CurrentScan.BinaryDataOffset[0]+this.MzF.CurrentScan.BinaryDataLength[0] + 9;
      }
      else { //finding second binary element when no BinaryDataLength - large read from BinaryDataOffset of the first binary element
       this.Position = +this.MzF.CurrentScan.BinaryDataOffset[0];
   ...
   if (this.MzF.CurrentScan.BinaryDataOffset[1]) {
    if (this.MzF.CurrentScan.PrefetchSpectrum) {
     this.MzF.fetchSpectrumData();
    }
    else {
     if (this.Internal.FetchAll && this.getNextScanNumber(this.CurrentScan.ScanNumber)) {
      this.Scans[this.CurrentScan.ScanNumber].Scan = this.CurrentScan;
      this.Reader.TextBuffer = "";
      this.Reader.readText(
       filetype.mzML.parseScanHeader,
       this.Scans[this.getNextScanNumber(this.CurrentScan.ScanNumber)].Offset,
       HEADER_SLICE_SIZE
      );  
     }
     else {
      delete this.Internal.FetchAll;
      this.MzF.Progress = 100;
      this.MzF.Ready = 1;
     }
    }
    ...
    this.TextBuffer = this.MzF.CurrentScan.BinaryDataOffset[0] ? "" : text.substr(end_ele_index)
    this.readText(
     filetype.mzML.parseScanHeader,
     this.Position,
     this.MzF.CurrentScan.BinaryDataOffset[0] ? MZML_SPECTRUM_SLICE_SIZE : HEADER_SLICE_SIZE
    );
   }
  },
   ...
   if (binary_index >= 0) {
    ...
    if (!this.firstBinaryArray) {
     ...
     this.readText(
      ...
      this.MzF.CurrentScan.BinaryDataOffset[1],
      this.MzF.CurrentScan.BinaryDataLength[1] ? this.MzF.CurrentScan.BinaryDataLength[1] + 10 : MZML_SPECTRUM_SLICE_SIZE
     );
    }
    else {
     console.log(this.firstBinaryArray);
     console.log(text);
     var a = decodeByteArray(this.firstBinaryArray,this.MzF.CurrentScan.CompressionType[0],this.MzF.CurrentScan.BinaryDataPrecision[0]);
     var b = decodeByteArray(text,this.MzF.CurrentScan.CompressionType[1],this.MzF.CurrentScan.BinaryDataPrecision[1]);
     if (this.MzF.CurrentScan.BinaryDataID[0] == "mz" && this.MzF.CurrentScan.BinaryDataID[1] == "int") {
      this.MzF.CurrentScan.SpectrumData.mzs = a;
      this.MzF.CurrentScan.SpectrumData.ints = b;
     }
     else if (this.MzF.CurrentScan.BinaryDataID[0] == "int" && this.MzF.CurrentScan.BinaryDataID[1] == "mz") {
      this.MzF.CurrentScan.SpectrumData.mzs = b;
      this.MzF.CurrentScan.SpectrumData.ints = a;
     }
     else {
...
//  },
  parseScanHeader : function() {
   ...
   for (var i = 0; i < eles.length-1; i++) {
    ...
    if (/<scan /.exec(eles[i])) {
     ...
     if (regexmatch) {
      this.MzF.CurrentScan.Centroided = +regexmatch[1];
     }
     ...
     if (regexmatch) {
      this.MzF.CurrentScan.ActivationMethod = regexmatch[1];
     }
    }
     ...
     if (regexmatch) {
      this.MzF.CurrentScan.CompressionType.push(regexmatch[1]);
     }
     regexmatch = /compressedLen="(\d+)"/.exec(eles[i]);
     if (regexmatch) {
      this.MzF.CurrentScan.BinaryDataLength.push(4*Math.ceil(regexmatch[1]/3)); //compressedLen is length after compression but before base64 encoding, need to convert
     }
     regexmatch = /precision="(\d+)"/.exec(eles[i]);
     if (regexmatch) {
      this.MzF.CurrentScan.BinaryDataPrecision.push(+regexmatch[1]);
     }
     this.MzF.CurrentScan.BinaryDataID.push('mz-int');
     this.MzF.CurrentScan.BinaryDataOffset.push(this.Position - text.length + eles.slice(0,i+1).join("").length + i + 1)   
     break;
    }
   }
   if (this.MzF.CurrentScan.BinaryDataOffset[0]) {
    if (this.MzF.CurrentScan.PrefetchSpectrum) {
     this.MzF.fetchSpectrumData();
    }
    else {
  ...
  parseSpectrumData : function() {
   ...
   if (end_peaks_index >= 0) {
    ...
    text = text.replace(/\n|\r/gm,"");
    var values = decodeByteArray(text,this.MzF.CurrentScan.CompressionType[0],this.MzF.CurrentScan.BinaryDataPrecision[0])
    this.MzF.CurrentScan.SpectrumData.mzs = [];
    this.MzF.CurrentScan.SpectrumData.ints = [];
    if (this.MzF.CurrentScan.BinaryDataID[0] == "mz-int") {
     for (var i = 0; i < values.length; i = i+2) { 
      this.MzF.CurrentScan.SpectrumData.mzs.push(values[i]);
      this.MzF.CurrentScan.SpectrumData.ints.push(values[i+1])
     }
    }
    else {
