...
HML.Quantifier = function(){
 ...
 var MS1Feature = function(MsDataFile,Extractions,PpmError,RTStart,RTEnd,EnforceOrder,MaxShift) {
  this.Ready = 0;
  this.Progress = 0;
  ...
  this.ExtractionMinMz -= HML.Math.ppmError(this.ExtractionMinMz,this.PpmError)*2;
  this.ExtractionMaxMz = Math.max.apply(null,all_max_mzs);
  this.ExtractionMaxMz += HML.Math.ppmError(this.ExtractionMaxMz,this.PpmError)*2;
  ...
  var getFirstScan = (function() {
   ...
   HML.Common.waitUntil(MsReady,getNextScan);
  }).bind(this);
 
  var getNextScan = (function() {
   var cs = this.MsDataFile.CurrentScan;
   this.CroppedRawSpectra[cs.ScanNumber] =  new HML.Data.Spectrum(cs.SpectrumData.mzs,cs.SpectrumData.ints).getCroppedSpectrum(this.ExtractionMinMz,this.ExtractionMaxMz);
    ...
    this.Extractions[label].forEach(function(TheoreticalSpectrum,TSi) {
     ...
     this.CroppedRawSpectra.forEach(function(ExSp,curr_scan) {
      ...
      TheoreticalSpectrum.mzs.forEach(function(mz) {
       var mzPpmErr = HML.Math.ppmError(mz,this.PpmError);
       ExInts.push(ExSp.getCroppedSpectrum(mz-mzPpmErr,mz+mzPpmErr).getTotalIntensity())
      },this);
      var croppedSpectrum = new HML.Data.Spectrum(TheoreticalSpectrum.mzs,ExInts);
...
//     var is_max = HML.Math.maxima(HML.Math.movingAverage(xic,xic.length/10));
     var is_max = HML.Math.maxima(xic);
     var TSPermutations = TheoreticalSpectrum.ints.map(function(int,i) {
      var ints = excludeIndex(TheoreticalSpectrum.ints,i);
      var permutation = new HML.Data.Spectrum(excludeIndex(TheoreticalSpectrum.mzs,i),ints);
     ...
     var bestMaximum = this.ExtractionMatches[label][TSi].Spectra.reduce((function(running_best,spectrum,i) {
      if (is_max[i]) {
       if (this.EnforceOrder && this.LastQuantRT) {
        if (this.EnforceOrder == 1 && (spectrum.RT < this.LastQuantRT)) return(running_best);
        if (this.EnforceOrder == -1 && (spectrum.RT > this.LastQuantRT)) return(running_best);
       }
       ...
       var bestPermutation = TSPermutations.reduce(function(running_best_permutatation,TSP) {
        ...
        var permutatationSpectrum = new HML.Data.Spectrum(excludeIndex(spectrum.mzs,TSP.ExcludedIndex),excludeIndex(spectrum.ints,TSP.ExcludedIndex));
    ...
    if (bestMatchedDist[1]) {
     ...
     this.Quant[label].MatchedDistributionTitle = this.Extractions[label][this.ExtractionMatches[label][bestMatchedDist[0]].ApexSpectrum.TSi].Title;
     ...
     this.Quant[label].QuantFailed = true;
    }
   },this);
   this.Ready = 1;
   this.Progress = 100;
  ...
  HML.Common.waitUntil(MsReady,getFirstScan);
 }
