...
HML.Plot = function() {
 ...
 var MS1FeatureRelAbundance = function(feature,labels) {
  ...
  var quant = labels.map(function(label) {return feature.Quant[label].QuantFailed ? 0 : feature.Quant[label].PermutationNormalisedIntensity});
 ...
 var MS1FeatureChromatogram = function(feature,labels) {
  ...
  var qe = [];   
  chromatograms.forEach(function(chr) {
   ...
   var txt = new fabric.Text(chr.Label+" ("+chr.QERT.toFixed(2)+")", {fill: colours[0], textAlign: "left", fontSize: labelfontsize, fontFamily:labelfontfamily, left: gleft+((chr.QERT-xmin)*xscale), top: gtop+gheight-ymax*yscale-labeloffset, originX: "left", originY: "top"});
   txt.set({top: txt.top-(txt.height/2), angle: -45});
   qe.push(txt);
  });
 ...
 var MS1FeatureSpectrum = function(feature,label) {
  ...
  var titletxt = new fabric.Text("Label: "+feature.Quant[label].MatchedDistributionTitle+"   -   idotp: "+feature.Quant[label].SpectrumMatchScore.toFixed(2), {textAlign: "center", fontSize: tickfontsize, fontFamily:tickfontfamily, left: gleft+(xmax-xmin)/2*xscale, top: gtop+gheight+tickmarkerlength*3, originX: "left", originY: "top"});
